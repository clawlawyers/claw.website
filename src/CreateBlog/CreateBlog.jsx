    ...
    return (
        <div className={Styles.main}>
            <BlockNoteView
                ...
                editor={editor}
                editable
                onChange={async () => { const p = await editor.blocksToHTMLLossy(editor.document); setHtml(p); setContent(editor.document) }}
            />
            <form style={{ display: "flex", flexDirection: "column", gap: 4, width: "100%" }} onSubmit={onPublish}>

                <label>Published Heading</label>
                <input
                    style={{ width: "80%", background: "#32353c", color: "white", padding: 10, fontSize: 15, borderRadius: 2, outline: "none", border: "none" }}
